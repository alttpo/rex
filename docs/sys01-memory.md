...
--
    ...
    FC: enter program
        input payload is a sequence of [PRGM]-denoted commands
        every command's inputs are validated before execution begins
        in: {
            struct {
                u8      opcode;
                frame   in[...];            // command's "in" data encoded in one or more frames
            } commands[...];                // program contents, limit 4096 bytes
        }
        out: {
            struct {
                u8      error;              // error code (check for non-zero)
            } errors[...];                  // same size as in.commands[]
        }
        NOTE: frames are used to allow for dynamic sizing of input program data; these frames are encoded within
        the already-framed message data, i.e. framing within framing.

    FB: execute entered program
        output payload is the sequence of all executed command output responses
        first command with an error stops the program
            the error code is set in the msgResponseHeader
            all command responses up to and including the errored command are included
        in: {
        }
        out: {
            struct {
                u8      opcode;             // opcode of command
                frame   out[...];           // command response data encoded in one or more frames
            } responses[...];
        }
